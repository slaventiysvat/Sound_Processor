...
//Need to fix this method (allocation memory for structure inside function)
...
/*returning dimension of new matrix*/
...
matrix_array* matrix_create(int rows, int columns) {
    ...
    matrix_array* new_matrix = (matrix_array*)malloc(sizeof(matrix_array));
...
void matrix_delete(matrix_array* unuseful_array) {
    ...
    memory_deallocate1D(unuseful_array->s_vector);
    memory_deallocate1D(unuseful_array);
}
...
/*Matrix will be multiply by constant*/
int vector_mpy_const(matrix_array* vector_in, matrix_array* vector_out, float constant) {


    int answer = 0;
    int columns = vector_in->columns;
    int rows = vector_in->rows;
    if (columns == 0) {
    ...
    int m = vector_in->columns;
    int n = vector_in->rows;
    int i = 0;
    int j = 0;
    float* pvector1 = vector_in->s_vector;
    float* pvector2 = vector_out->s_vector;
    for (i; i < n; i++) {

        float* pvector_tmp_1 = pvector1 + (i * m);
        float* pvector_tmp_2 = pvector2 + (i * m);
        for (j; j < m; j++) {

            *(pvector_tmp_2 + j) = *(pvector_tmp_1 + j) * constant;
...
/*Matrix will be divide by constant*/
